...
hour_df['month'] = hour_df['month'].map({1: 'Jan', 2: 'Feb', 3: 'Mar', 4: 'Apr', 5: 'May', 6: 'Jun', 7: 'Jul', 8: 'Aug', 9: 'Sep', 10: 'Oct', 11: 'Nov', 12: 'Des'})

hour_df['weekday'] = hour_df['weekday'].map({0: 'Sun', 1: 'Mon', 2: 'Tue', 3: 'Wed', 4: 'Thu', 5: 'Fri', 6: 'Sat'})

hour_df['weather'] = hour_df['weather'].map({1: 'Clear', 2: 'Mist/Cloudy', 3: 'Light Snow/Rain', 4: 'Heavy Rain'})
...
st.header("Penyewa Sepeda Berdasarkan Durasi (Jam)")
data = hour_df.groupby('hour')[['count']].sum().reset_index()
fig_hourly_histogram = plt.figure(figsize=(10, 6))
sns.histplot(data=data, x='hour', weights='count', binwidth=1, color='blue', edgecolor='black', stat='count', kde=True)
plt.xticks(range(min(data['hour']), max(data['hour']) + 1))
plt.xlabel('Jam')
plt.ylabel('Jumlah Penyewa')
st.pyplot(fig_hourly_histogram)
